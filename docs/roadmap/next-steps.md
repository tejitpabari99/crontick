...
Branch analyzed: `users/tejitpabari/prompt-cron-daemon-autostart`  
Date: 2026-07-25
...
These are near-term hardening tasks before larger AI-first feature work. `multi-persona-review.v1.model.md` is the only review artifact under `.dev\users-tejitpabari-prompt-cron-daemon-autostart\` that contains explicit DEFER decisions; no separate rubber-duck artifact is present in that folder, so the source-observed gaps below are treated as rubber-duck/known-gap follow-ups.

| Priority | Item                                                                           | Problem                                                                                                                                                                                                                                                                                                                 | Proposed approach                                                                                                                                                                                                                                | Effort | Dependencies                                   |
| -------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -----: | ---------------------------------------------- |
| P0       | Decide how to handle archived startup-registration references                  | Review deferred A5: archived `docs\plan\v1` still contains old HKCU/systemd/launchd research, while runtime/public surfaces removed those features (`.dev\...\multi-persona-review.v1.model.md:21`, `.dev\...\investigate.v1.model.md:167-179`). For public release, stale archives can confuse users and contributors. | Add an explicit "historical / superseded" banner to archived plan docs, or move them under a clearly archival folder excluded from public docs navigation. Do not reintroduce startup-registration product promises.                             |      S | None                                           |
...
| P0       | Make the public client option name friendlier without using banned terminology | Review deferred R2: `CrontickClientOptions` inherits `allowStart` from ensure (`src\client.ts:15-18`, `.dev\...\multi-persona-review.v1.model.md:28`). That name is implementation-centric.                                                                                                                             | Add a backwards-compatible alias such as `startDaemon?: boolean` that maps to `allowStart`, then document it. Avoid `autoStart` if removal guards still prohibit that lexeme.                                                                    |      S | Config/client docs                             |
| P0       | Fix generated JSON schema before public docs rely on it                        | `src\schemas\job.schema.json` is effectively empty (`$ref` to an empty `CrontickJob`) while MCP exposes a live generated schema resource (`src\mcp\index.ts:702-719`). Public users will expect a usable schema file.                                                                                                   | Repair `scripts\generate-schema.mjs` / zod-to-json-schema generation, add a test that the checked-in schema contains schedule/action definitions, and reference it from docs.                                                                    |      M | None                                           |
...
- [ ] Fix checked-in job JSON schema before publishing schema docs.
...
- [ ] Keep archived plan docs either hidden or clearly marked superseded.
...
- Resolve deferred review items: archived-doc stance, daemon supervision docs, `startDaemon` client alias.
- Fix generated job schema.
