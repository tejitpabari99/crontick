...
Completed hardening items removed from this table: daemon demand-start docs, the single `startDaemon`
option, core-generated per-job schema sidecars, CLI log-follow removal, deterministic prompt session
reuse behavior, and token-budget field removal.
...
- Done: clarified prompt/session reuse behavior and removed speculative token-budget fields.
